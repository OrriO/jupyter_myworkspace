...
function recommend() {
	$.ajax({
		type: 'GET',
		url: 'http://www.technologyx.cn/movie/getbyuser',
		dataType:'jsonp',
		data:{
			'size':50,
			'start':start,
...
function appendHTML(records, is_rec) {
   ...
   for(var i = 0; i < records.length; i++ ){
		...
		if(movie_id_list.indexOf(movid_id) < 0){
			movie_id_list.push(movid_id)
        	hts[i]=$('<div class="item" onclick="javascript:toOneMovie(\'' + movid_id + '\')" >' +
        			...
        			records[i].name + '   <p style="color:red;display:inline;">' + records[i].rat + '</p>' +
        			'</div>' + 
				'</div>')[0];
		}
    }
